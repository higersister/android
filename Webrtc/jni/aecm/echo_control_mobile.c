                                ...
                                int16_t nrOfSamples)
{
    ...
    if (aecm == NULL)
    {
        ...
        LOGD("$$$$$$$$$$WebRtcAecm_BufferFarend->(aecm == null)");
        return -1;
    }

    if (farend == NULL)
    {
        LOGD("$$$$$$$$$$WebRtcAecm_BufferFarend->(farend == NULL)");
    ...
    {
        LOGD("$$$$$$$$$$WebRtcAecm_BufferFarend->(aecm->initFlag != kInitCheck)");
    ...
    {
        LOGD("$$$$$$$$$$WebRtcAecm_BufferFarend->(nrOfSamples != 80 && nrOfSamples != 160)");
        aecm->lastError = AECM_BAD_PARAMETER_ERROR;
        return -1;
    }
    ...
    if (!aecm->ECstartup)
    {
        LOGD("$$$$$$$$$$WebRtcAecm_BufferFarend->(!aecm->ECstartup)");
        WebRtcAecm_DelayComp(aecm);
    }

    WebRtc_WriteBuffer(aecm->farendBuf, farend, (size_t) nrOfSamples);
    LOGD("$$$$$$$$$$WebRtcAecm_BufferFarend->WebRtc_WriteBuffer");
    return retVal;
}
...
int32_t WebRtcAecm_set_config(void *aecmInst, AecmConfig config)
{
    ...
    if (aecm == NULL)
    {
        ...
        LOGD("$$$$$$$$$$WebRtcAecm_set_config->(aecm == NULL)");
        return -1;
    }

    if (aecm->initFlag != kInitCheck)
    {
        LOGD("$$$$$$$$$$WebRtcAecm_set_config->(aecm->initFlag != kInitCheck)");
        aecm->lastError = AECM_UNINITIALIZED_ERROR;
        return -1;
    }
    ...
    {
        LOGD("$$$$$$$$$$WebRtcAecm_set_config->(config.cngMode != AecmFalse && config.cngMode != AecmTrue)");
        aecm->lastError = AECM_BAD_PARAMETER_ERROR;
        return -1;
    }
    aecm->aecmCore->cngMode = config.cngMode;
    LOGD("$$$$$$$$$$WebRtcAecm_set_config->config.cngMode:%hd",config.cngMode);
    ...
    LOGD("$$$$$$$$$$WebRtcAecm_set_config->config.echoMode:%hd",config.echoMode);
    if (aecm->echoMode == 0)
    {
